...
class StockChartApp {
    ...
    updateChartData() {
        ...
        this.chart.data.datasets[0].pointBackgroundColor = this.chart.data.labels.map(date => {
            return newsDates.includes(date) ? 'rgba(59, 130, 246, 1)' : 'rgba(255, 99, 132, 1)';
        });
        
        this.chart.data.datasets[0].pointBorderColor = this.chart.data.labels.map(date => {
            return newsDates.includes(date) ? 'rgba(59, 130, 246, 1)' : 'rgba(255, 99, 132, 1)';
        });
